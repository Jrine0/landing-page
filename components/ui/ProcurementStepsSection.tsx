...
import { FileSearch, GitMerge, BarChart2 } from "lucide-react";
...
function StepVisualCard({ visual }: { visual: (typeof STEPS)[0]["visual"] }) {
  return (
    <div
      className="w-full bg-[#edf1f8] border border-[#d4dce8] p-4 sm:p-5 lg:p-7"
      style={{ borderRadius: "2px" }}
    >
      <div
        className="bg-white border border-[#e2e8f0] shadow-[0_2px_12px_rgba(31,58,97,0.06)] p-4 sm:p-5 lg:p-6"
        style={{ borderRadius: "2px" }}
      >
        {/* Card header */}
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-[#edf1f8]">
        ...
        <div className="space-y-0">
          {visual.rows.map(({ label, value }) => (
            <div
              key={label}
              className="flex items-center justify-between gap-4 py-2 border-b border-[#f4f7fb] last:border-0"
            >
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center shrink-0">
        <div
          className="relative z-10 w-10 h-10 sm:w-11 sm:h-11 bg-white border border-[#d4dce8] shadow-[0_2px_8px_rgba(31,58,97,0.07)] flex items-center justify-center"
          style={{ borderRadius: "2px" }}
        >
          <step.Icon className="h-4 w-4 sm:h-5 sm:w-5 text-[#496c83]" />
        </div>
        {/* Connector — hidden after last step */}
        {!isLast && <div className="flex-1 w-px bg-[#d4dce8] mt-2" />}
      </div>

      {/* ── Content: visual card + text ── */}
      <div
        className={`flex-1 grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6 lg:gap-12 items-center ${
          isLast ? "pb-0" : "pb-12 sm:pb-14 lg:pb-20"
        }`}
      >
        {/* Visual card — full width on mobile, left column on desktop */}
        <StepVisualCard visual={step.visual} />

        {/* Text — full width on mobile, right column on desktop */}
        <div className="lg:pl-2">
          ...
          <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-[#1f3a61] mb-3 tracking-tight leading-tight">
            {step.title}
          </h3>
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        >
          ...
          <p className="text-base sm:text-lg max-w-xl mx-auto text-[#496c83]">
            Healthcare procurement becomes a structured, auditable workflow
